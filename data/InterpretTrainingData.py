...
INTERPRETED_DIR = DATA_DIR / "interpreted"
...
def main() -> None:
    ...
    combined_rows = [*poker_now_rows, *kaggle_rows]
    write_rows(INTERPRETED_DIR / "ranging_rows.csv", combined_rows)
    write_rows(
        INTERPRETED_DIR / "preflop_ranging_rows.csv",
        [row for row in combined_rows if row.get("street") == "preflop"],
    )
    write_rows(
        INTERPRETED_DIR / "postflop_ranging_rows.csv",
        [row for row in combined_rows if row.get("street") != "preflop"],
    )

    print(f"wrote {len(poker_now_rows)} PokerNow rows")
    print(f"wrote {len(kaggle_rows)} Kaggle/HM rows")
    print(f"wrote {len(combined_rows)} combined rows")
...
def fill_keras_equity(label: str, batch_size: int = 4096) -> None:
    ...
    for start in range(0, total, batch_size):
        ...
        for (row, _, _, _), prediction in zip(batch, predictions):
            ...
            row["call_ev"] = row_call_ev(row)
        print(f"{label}: filled {min(start + batch_size, total)} / {total} rows")

    _EQUITY_JOBS.clear()


def row_call_ev(row: dict) -> float:
    try:
        keras_equity = float(row.get("keras_equity") or 0.0)
        facing_amount_bb = float(row.get("facing_amount_bb") or 0.0)
        pot_bb = float(row.get("pot_bb") or 0.0)
    except (TypeError, ValueError):
        return 0.0

    return keras_equity * (pot_bb + facing_amount_bb) - facing_amount_bb
...
def parse_kaggle_hand(path: Path, hand_text: str, fallback_number: int, starting_player_count: int) -> list[dict]:
    ...
    for line in lines:
        ...
        action_match = re.match(r"Player (.+) (folds|checks|calls|bets|raises|allin)(?: \(([0-9.]+)\))?", line)
        if not action_match:
            continue
        ...
        if raw_action == "folds":
            ...
            pot += contribution
        elif raw_action in {"bets", "raises", "allin"}:
            contribution = amount
            current_street_bets[seat] += contribution
            players[seat]["bet"] += contribution
            players[seat]["stack"] -= contribution
...
def make_row(
    ...
    board_cards = [
        str(state.community_cards[index]) if index < len(state.community_cards) else ""
        for index in range(5)
    ]
    row = {
        "hole_card_1": str(hole_cards[0]),
        "hole_card_2": str(hole_cards[1]),
        "board_card_1": board_cards[0],
        "board_card_2": board_cards[1],
        "board_card_3": board_cards[2],
        "board_card_4": board_cards[3],
        "board_card_5": board_cards[4],
