...
class CatBoostPokerSimulator:
    ...
    def choose_action(self, player: SimPlayer, facing: float) -> str:
        ...
        context = ActionContext(
            ...
            street_action_number=self.street_action_number,
            last_aggressor_name=self.players[self.last_aggressor_seat].name if self.last_aggressor_seat is not None else None,
            can_check=facing <= 0,
            can_raise=self.can_raise(player, facing),
            is_facing_all_in=any(other.all_in and other.street_bet >= player.street_bet + facing for other in self.players if other.seat != player.seat),
        )
